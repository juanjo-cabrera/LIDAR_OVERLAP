...
def main():
    ...
    if saved_overlap == True:
        ...
        x_idx, y_idx = keyframe_manager.compute_centroid(scan_idx)
        for i in range(0, len(scan_times)):
            ...
            x_i, y_i = keyframe_manager.compute_centroid(i)
            ...
            for gamma in gammas:
                ...
                overlap_i = np.count_nonzero(
                    abs(reference_range[reference_range > 0] - current_range[reference_range > 0]) < 6) / valid_num
            ...
            indices = np.argsort(overlaps_i)
            n_largest_values = 2
            max_overlap_idx = indices[::-1][:n_largest_values]
            gammas_selected = gammas[max_overlap_idx]

            overlaps_j = []
            for gamma in gammas_selected:
                ...
                overlap_j = np.count_nonzero(abs(reference_range[reference_range > 0] - current_range[reference_range > 0]) < 1) / valid_num
                overlaps_j.append(overlap_j)

            overlap = np.max(overlaps_j)
        ...
        xys = gt_pos[:, 0:2]
        with open("metodo21_overlaps_pi_sextos", "wb") as fp:
