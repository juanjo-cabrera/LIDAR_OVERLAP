...
def main():
    ...
    # keyframe_manager.keyframes[scan_idx].pre_process()
    # keyframe_manager.keyframes[scan_idx].normalize2center()
    # centroid(keyframe_manager)
    ...
    if saved_overlap == True:
        ...
        # keyframe_manager.compute_normalization2maxbound(scan_idx)
        keyframe_manager.compute_normalization2corner(scan_idx, corner=1)
        # keyframe_manager.compute_bbox(scan_idx)
        ...
        for i in range(0, len(scan_times)):
            ...
            # keyframe_manager.keyframes[i].normalize2center()
            for j in range(0, 3):
                print('iter: ', j)
                # if j == 0:
                #     keyframe_manager.compute_normalization2minbound(i)
                # elif j == 1:
                #     keyframe_manager.compute_normalization2minbound(i)
                keyframe_manager.compute_normalization2corner(i, corner=j)

                # keyframe_manager.compute_bbox(i)

                # x_i, y_i = keyframe_manager.compute_centroid(i, pcd_format='normalized')
                #
                #
                # tx = x_idx - x_i
                # ty = y_idx - y_i
                ...
                for gamma in gammas:
                    ...
                    if debug:
                        keyframe_manager.keyframes[i].draw_pointclouds_bbox(keyframe_manager.keyframes[scan_idx], transformation_matrix)
                    ...
                    reference_points_in_current = np.dot(transformation_matrix, reference_homogeneous_points.T).T
            ...
            max_overlap_idx = np.argmax(overlaps_i)
            gamma = gammas[max_overlap_idx]
            transformation_matrix = HomogeneousMatrix(np.array([tx, ty, tz]), Euler(np.array([alpha, beta, gamma])))
            transformation_matrix = transformation_matrix.array
            ...
            overlap = np.count_nonzero(abs(reference_range[reference_range > 0] - current_range[reference_range > 0]) < 1) / valid_num
        ...
        xys = gt_pos[:, 0:2]
        with open("metodo22_overlaps_pi_sextos", "wb") as fp:
